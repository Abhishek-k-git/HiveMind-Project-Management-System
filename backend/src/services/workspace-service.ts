...
export const createWorkspaceService = async (
  ...
  const ownerRole = await PermissionModel.findOne({ name: Roles.OWNER });
...
export const getWorkspaceMembersService = async (workspaceId: string) => {
  ...
  const roles = await PermissionModel.find({}, { name: 1, _id: 1 })
    .select("-permission")
    .lean();
...
export const changeMemberRoleService = async (
  ...
  const role = await PermissionModel.findById(roleId);
  if (!role) {
